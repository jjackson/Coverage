...
def generate_flw_summary(coverage_data):
    ...
    # Step 1: Merge DU status from DU table
    du_status_lookup = coverage_data.delivery_units_df[['du_name', 'du_status']].dropna(subset=['du_name'])
    service_df = service_df.merge(du_status_lookup, on='du_name', how='left')
    ...
    summary.fillna({'total_dus_completed': 0, 'total_dus_visited': 0}, inplace=True)

...
LEFT JOIN users_user u 
    ON uv.user_id = u.id
WHERE 
  oo.id IN (516,517,531,539,566,575,601,603,722)
...
WHERE oo.id IN (516,517,531,539,566,575,601,603,722)
AND oo.is_test = 'false'
...
"flw_data_quality_analysis_batch" : """SELECT 
    opportunity_opportunity.name AS opportunity_name,
    opportunity_uservisit.id AS visit_id,
    ...
    users_user.username,
    opportunity_deliverunit.name AS unit_name,
    ...
    form_json -> 'form' -> 'additional_case_info' ->> 'household_phone' AS phone_number,
    ...
    form_json -> 'form' -> 'muac_group' -> 'Muac' -> 'vitals' ->> 'muac_colour' AS muac_vitals_color,
    ...
    form_json -> 'form' -> 'case' -> 'update' ->> 'under_treatment_for_mal' AS under_malnutrition_treatment,
...
WHERE opportunity_opportunity.id = {opp_id}
